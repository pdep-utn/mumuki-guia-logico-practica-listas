...
test(peru_esta_en_la_a) :-
  estaEn(peru, a).
test(once_esta_en_la_h) :-
  estaEn(once, h).


test(distancia_entre_peru_y_primeraJunta_es_3) :-
  distancia(peru, primaraJunta, Distancia),
  assertion(Distancia == 3).

test(pellegrini_independencia_misma_altura) :-
  mismaAltura(pellegrini, independencia).

test(bolivar_pichincha_viaje_facil) :-
  viajeFacil(bolivar, pichincha).
test(lima_independenciaC_viaje_facil) :-
